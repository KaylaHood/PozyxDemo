...
int PozyxClass::getAnchorIds(uint16_t anchors[], int size, uint16_t remote_id)
{
	...
	if (status == POZYX_FAILURE || list_size < size) {
#if KAYLA_DEBUG 
		Serial.println("getAnchorIds failed upon \"getDeviceListSize\"");
#endif
		return POZYX_FAILURE;
	}
#if KAYLA_DEBUG
		...
		Serial.println(" devices in the internal list");
	}
#endif
	...
	if (remote_id == NULL) {
...
#if KAYLA_DEBUG
		if (status == POZYX_FAILURE) {
			Serial.println("getAnchorIds failed upon \"regFunction(POZYX_DEVICES_GETIDS)\"");
		}
		else {
			Serial.print("Device IDs in internal list: ");
		...
		if (status == POZYX_FAILURE) {
#if KAYLA_DEBUG
			Serial.println("getAnchorIds failed upon \"remoteRegFunction(POZYX_DEVICES_GETIDS)\"");
#endif
		}
	}
	...
	if (status == POZYX_SUCCESS) {
		...
		for (int i = 0; i < list_size; i++) {
			...
			if (data[2] == POZYX_ANCHOR_MODE + 8) {
				anchors[j] = devices[i];
				j++;
			}
#if KAYLA_DEBUG
			Serial.print("Device with ID 0x");
			Serial.print(devices[i], HEX);
			Serial.print(" has T/A description of 0b");
		...
		if (j < size) {
#if KAYLA_DEBUG
			Serial.print("getAnchorIds failed: ");
			Serial.print("found ");
			Serial.print(j);
			Serial.print(" out of ");
			Serial.print(size);
...
int PozyxClass::doAnchorCalibration(int dimension, int num_measurements, int num_anchors, uint16_t anchors[], int32_t heights[])
{
...
#if KAYLA_DEBUG
			Serial.print("Adding anchor with ID 0x");
			Serial.print(anchor.network_id, HEX);
			Serial.println(" to internal anchor list");
#endif
	...
	status = regFunction(POZYX_DEVICES_CALIBRATE, (uint8_t *)&params, 2, NULL, 0);
	delay(1000);
	if (status != POZYX_SUCCESS) {
#if KAYLA_DEBUG
		Serial.println("register function \"POZYX_DEVICES_CALIBRATE\" malfunctioned");
#endif
		return POZYX_FAILURE;
	}

#if KAYLA_DEBUG
	Serial.println("Waiting for flag response...");
#endif
	...
	if (waitForFlag_safe(POZYX_INT_STATUS_FUNC | POZYX_INT_STATUS_ERR, 25000, &int_status)) {
		if ((int_status & POZYX_INT_STATUS_ERR) == POZYX_INT_STATUS_ERR)
		{
			...
			// Please read out the register POZYX_ERRORCODE to obtain more information about the error
#if KAYLA_DEBUG
			Serial.println("Flag received was negative: an error occurred during auto calibration");
#endif
			return POZYX_FAILURE;
		}
		else {
#if KAYLA_DEBUG
			Serial.println("Flag received was positive: auto calibration successful");
#endif
			return POZYX_SUCCESS;
		}
	}
...
#if KAYLA_DEBUG
		Serial.println("No flag received: auto calibration timed out");
#endif
		return POZYX_TIMEOUT;
	}
